...
foreach ($cmd in $commands) {
  switch ([string]$cmd.type) {
    ...
    'point' { $s=[Windows.Shapes.Ellipse]::new(); $r=([double]($cmd.size ?? 1))/2; $s.Width=$r*2; $s.Height=$r*2; $s.Fill=Brush $fill; Add-Shape $s ([double]$cmd.x-$r) ([double]$cmd.y-$r) }
...
function Text-For($p, $st) { if ($p.bind) { $v = $st.values.($p.bind); if ($null -eq $v) { return '' }; return [string]$v }; if ($null -eq $p.text) { return '' }; return [string]$p.text }
...
function Render-Node($n, $st) {
  ...
  switch ([string]$n.type) {
    ...
    'heading' { $tb=[Windows.Controls.TextBlock]::new(); $tb.Text=Text-For $p $st; $tb.FontWeight='SemiBold'; $tb.FontSize=(26 - ([int]($p.level ?? 1)*3)); $tb.Margin='0,4,0,6'; return $tb }
    ...
    'spacer' { $b=[Windows.Controls.Border]::new(); $b.Height=[double]($p.size ?? 8); return $b }
    'button' { $b=[Windows.Controls.Button]::new(); $b.Content=[string]($p.label ?? 'Button'); $b.Margin='0,4,6,4'; $b.Padding='12,5,12,5'; $hid=$n.handlers.click; if ($hid -ne $null) { $b.Add_Click({ try { Invoke-RestMethod -Uri ($base + '/click?id=' + $hid) -Method Post -UseBasicParsing | Out-Null; Render-Root } catch {} }) }; return $b }
    'input' { $x=[Windows.Controls.TextBox]::new(); $x.Text=[string]($st.values.($p.bind) ?? ''); $x.Margin='0,4,0,4'; $bind=[string]$p.bind; $x.Add_LostFocus({ if ($bind) { try { Invoke-RestMethod -Uri ($base + '/set?k=' + [uri]::EscapeDataString($bind) + '&v=' + [uri]::EscapeDataString($this.Text)) -Method Post -UseBasicParsing | Out-Null } catch {} } }); return $x }
    'textarea' { $x=[Windows.Controls.TextBox]::new(); $x.Text=[string]($st.values.($p.bind) ?? ''); $x.AcceptsReturn=$true; $x.Height=90; $x.TextWrapping='Wrap'; $x.Margin='0,4,0,4'; $bind=[string]$p.bind; $x.Add_LostFocus({ if ($bind) { try { Invoke-RestMethod -Uri ($base + '/set?k=' + [uri]::EscapeDataString($bind) + '&v=' + [uri]::EscapeDataString($this.Text)) -Method Post -UseBasicParsing | Out-Null } catch {} } }); return $x }
    'checkbox' { $c=[Windows.Controls.CheckBox]::new(); $c.Content=[string]$p.label; $c.IsChecked=[bool]($st.values.($p.bind)); $c.Margin='0,4,0,4'; $bind=[string]$p.bind; $c.Add_Click({ if ($bind) { try { Invoke-RestMethod -Uri ($base + '/set?k=' + [uri]::EscapeDataString($bind) + '&v=' + [uri]::EscapeDataString([string]$this.IsChecked)) -Method Post -UseBasicParsing | Out-Null } catch {} } }); return $c }
    'slider' { $s=[Windows.Controls.Slider]::new(); $s.Minimum=[double]($p.min ?? 0); $s.Maximum=[double]($p.max ?? 100); $s.Value=[double]($st.values.($p.bind) ?? $s.Minimum); $s.Margin='0,6,0,6'; $bind=[string]$p.bind; $s.Add_LostMouseCapture({ if ($bind) { try { Invoke-RestMethod -Uri ($base + '/set?k=' + [uri]::EscapeDataString($bind) + '&v=' + [uri]::EscapeDataString([string]$this.Value)) -Method Post -UseBasicParsing | Out-Null } catch {} } }); return $s }
    'select' { $cb=[Windows.Controls.ComboBox]::new(); foreach ($o in $p.options) { [void]$cb.Items.Add([string]$o) }; $cb.SelectedItem=[string]($st.values.($p.bind) ?? ''); $cb.Margin='0,4,0,4'; $bind=[string]$p.bind; $cb.Add_SelectionChanged({ if ($bind -and $this.SelectedItem) { try { Invoke-RestMethod -Uri ($base + '/set?k=' + [uri]::EscapeDataString($bind) + '&v=' + [uri]::EscapeDataString([string]$this.SelectedItem)) -Method Post -UseBasicParsing | Out-Null } catch {} } }); return $cb }
    'progress' { $pr=[Windows.Controls.ProgressBar]::new(); $pr.Minimum=0; $pr.Maximum=[double]($p.max ?? 100); $raw = if ($p.bind) { $st.values.($p.bind) } else { $p.value }; $pr.Value=[double]($raw ?? 0); $pr.Height=12; $pr.Margin='0,5,0,5'; return $pr }
    default { return $null }
  }
}
