...
const __sdevTranslator = (function () {
  ...
  // Levenshtein-based fuzzy matcher — handles typos, conjugations, synonyms.
  ...
  const FUZZY_REPLACERS = {};
  const IDENT_INTRODUCERS = new Set(['forge','be','new','essence','conjure','extend','summon','within']);
  function compileFuzzyReplacer(lang) {
    ...
    const fn = (src) => {
      ...
      return src.replace(wordRe, (word, offset) => {
        if (/^[\x00-\x7F]+$/.test(word)) { prevToken = word.toLowerCase(); return word; }
        const lower = word.toLowerCase();
        if (table[lower]) { const out = table[lower]; prevToken = out; return out; }
        const prevChar = offset > 0 ? src[offset-1] : '';
        if (prevChar === '.' || IDENT_INTRODUCERS.has(prevToken)) { prevToken = lower; return word; }
        if ([...lower].length < 4) { prevToken = lower; return word; }
        const threshold = [...lower].length >= 6 ? 2 : 1;
        const candidates = byChar.get(lower[0]) || [];
        let best = null;
        for (const k of candidates) {
          if (Math.abs(k.length - lower.length) > threshold) continue;
          const d = levenshtein(lower, k);
          if (d <= threshold && (!best || d < best.dist)) { best = { key: k, dist: d }; if (d === 0) break; }
        }
        if (best) { const out = table[best.key]; prevToken = out; return out; }
        prevToken = lower;
        return word;
      });
    };
  ...
  function detectLanguage(source) {
    ...
    for (const lang of SUPPORTED_LANGUAGES) {
      const table = KEYWORD_TABLES[lang];
      let score = 0;
      for (const word of Object.keys(table)) if (source.includes(word)) score++;
  ...
  function translateSource(source, sourceLanguage = 'auto') {
    ...
    const translated = segments.map(seg => {
      ...
      t = replace(t);
      t = fuzzy(t);
      // Context fix: `forge name(` → `conjure name(` (method declaration)
